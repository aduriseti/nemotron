"""
v3_freqord + all-three-pairs arity-2 contradiction pre-pruning for arity-3.

For each arity-3 triple (s0, s1, s2), all three pairwise sub-pairs were tried
in arity-2. Each failure recorded a witness conflict (pos_c, ex1, ex2).

Before running arity-3 phase-1, check all three conflicts in order:
  - (s0,s1) conflict resolved by s2? (c_ex1 != c_ex2 at pos_c)
  - (s0,s2) conflict resolved by s1? (b_ex1 != b_ex2 at pos_c)
  - (s1,s2) conflict resolved by s0? (a_ex1 != a_ex2 at pos_c)

Short-circuit: if any one is unresolved, skip the triple (O(1)).

Expected rejection rate: 1 - (0.5)^3 = ~87.5% of wrong triples skipped
before phase-1 runs, vs ~50% with a single pair check.
"""
...
def find_rule(
    ...
    # ── arity 3 (frequency-ordered; all-three-pairs a2 pre-pruning) ──────────
    for s0 in _S_ORDER_A3:
        rep0 = _TRANS_BYTES[_S_REP[s0]]
        for s1 in _S_ORDER_A3:
            rep1 = _TRANS_BYTES[_S_REP[s1]]

            conf_01 = a2_conflicts.get((s0, s1))
            ...
            for s2 in _S_ORDER_A3:
                ...
                # Check all three pairwise a2 conflicts. Short-circuit on the
                # first unresolved one — that guarantees a contradiction exists.
                pruned = False

                # (s0,s1) conflict: resolved only if s2 splits the witnesses
                if not pruned and conf_01 is not None:
                    pos_c, ex1_c, ex2_c = conf_01
                    if lo <= pos_c < safe_hi:
                        bit_checks += 2
                        if (rep2[in_bytes[ex1_c]] >> (7 - pos_c)) & 1 == \
                           (rep2[in_bytes[ex2_c]] >> (7 - pos_c)) & 1:
                            hot_pos = pos_c
                            pruned = True

                # (s0,s2) conflict: resolved only if s1 splits the witnesses
                if not pruned:
                    conf_02 = a2_conflicts.get((s0, s2))
                    if conf_02 is not None:
                        pos_c, ex1_c, ex2_c = conf_02
                        if lo <= pos_c < safe_hi:
                            bit_checks += 2
                            if (rep1[in_bytes[ex1_c]] >> (7 - pos_c)) & 1 == \
                               (rep1[in_bytes[ex2_c]] >> (7 - pos_c)) & 1:
                                hot_pos = pos_c
                                pruned = True

                # (s1,s2) conflict: resolved only if s0 splits the witnesses
                if not pruned:
                    conf_12 = a2_conflicts.get((s1, s2))
                    if conf_12 is not None:
                        pos_c, ex1_c, ex2_c = conf_12
                        if lo <= pos_c < safe_hi:
                            bit_checks += 2
                            if (rep0[in_bytes[ex1_c]] >> (7 - pos_c)) & 1 == \
                               (rep0[in_bytes[ex2_c]] >> (7 - pos_c)) & 1:
                                hot_pos = pos_c
                                pruned = True
