"""
infer3 + n_ex_limit: use only the first N examples in the search.

Ablation insight: with N=5 examples and unlimited budget, accuracy is 93%.
With all 10 examples and 5K budget, accuracy is only 71.3%.
Using fewer examples means the solver terminates earlier (less-constrained TT
is consistent sooner), at the cost of occasionally picking a wrong rule that
fits N examples but not all. This trades ambiguity-wrong-rate for budget-none-rate,
increasing net correct answers within 5K checks.

The default n_ex_limit is tuned by sweeping the full dataset.
"""
...
# Empirically derived from 300 benchmark problems — sorted by frequency descending.
# Arity-2: all 15 offsets (s=0 included last, rarely used)
_S_ORDER_A2 = [-1, -6, -2, -4, -3, 1, -5, 4, 5, -7, 6, 7, 2, 3, 0]
# Arity-3: 14 offsets (s=0 omitted — zero empirical occurrences)
...
def find_rule(
    ...
    # Limit examples used in the search — fewer examples means the solver finds
    # a consistent rule faster (less-constrained TT), staying within bit-check budget.
    if n_ex_limit > 0:
        in_bytes  = in_bytes[:n_ex_limit]
        out_bytes = out_bytes[:n_ex_limit]
    n_ex = len(in_bytes)
    bit_checks = 0
    ...
    # ── arity 2 (frequency-ordered s-values) ─────────────────────────────────
    ...
    # ── arity 3 (frequency-ordered s-values, s=0 omitted) ────────────────────
