"""
python_solver_arith_pm1_abs_cat.py — v7: branched from v6.

Promotes `cat` from target-only fast-path to a first-class example operator on
top of v6's {add, sub, mul, add1, addm1, mul1, mulm1, sub_abs, sub_neg_abs}.

cat output digits are exactly the 4 operand digits (in formatter-dependent
order), so column derivation is done by enumerating b0/b1 and verifying via
_derive_output (which already understands the formatter post-fn). The
algebraic-inversion path decomposes a 4-digit result into (L_top, R_bot).
"""
...
# add, sub, mul, sub_abs, addm1, cat, mul1, sub_neg_abs, add1, mulm1
OP_ORDER = ['add', 'sub', 'mul', 'sub_abs', 'addm1', 'cat',
            'mul1', 'sub_neg_abs', 'add1', 'mulm1']
