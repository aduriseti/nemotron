"""
Generate ops-per-puzzle distribution report for v3 / v5.
Writes markdown to v5_ops_distribution.md.

v5 adds add1, addm1, mul1, mulm1 support on top of v3 (add/sub/mul/cat).
"""
...
SUPPORTED_OPS = ('add', 'sub', 'mul', 'cat', 'add1', 'addm1', 'mul1', 'mulm1')
NEW_OPS = ('add1', 'addm1', 'mul1', 'mulm1')
INDEX_PATH = '/workspaces/nemotron/reasoners/cryptarithm_solver/op_index.json'
OUT_PATH = '/workspaces/nemotron/reasoners/cryptarithm_solver/v5_ops_distribution.md'
...
def main():
    ...
    lines = [
        ...
        f'Problems: {n} (add/sub/mul/cat/add1/addm1/mul1/mulm1 targets)', '',
    ...
    for op_name in SUPPORTED_OPS:
        rows_for_op = [op_name in {r['target_op'] for r in all_records['v3']}]
    ...
    lines += ['## Per-Puzzle Speedup (v5 vs v3)', '',
              '| Speedup bucket | Count | % |',
              '|----------------|-------|---|']
    speedups = []
    for r1, r5 in zip(all_records['v3'], all_records['v5']):
        if r1['ops'] > 0:
            speedups.append(r1['ops'] / max(r5['ops'], 1))
    ...
    for op_name in NEW_OPS:
        ...
        red = t3 / t5 if t5 else 0
        lines.append(
            f'| {op_name} | {c3}/{len(r3s)} | {c5}/{len(r5s)} | {c5-c3:+d} '
            f'| {t3:,} | {t5:,} | {red:.2f}x |'
        )
    lines += ['']

    regressions = [(r3, r5) for r3, r5 in zip(all_records['v3'], all_records['v5'])
                   if r3['correct'] and not r5['correct']]
    lines += [f'## Regressions (v3 correct, v5 wrong)', '',
              f'Total: {len(regressions)} of {v3_correct} v3-correct problems', '']
    if regressions:
        lines += ['| Problem ID | Target op | v3 ops | v5 ops |',
                  '|------------|-----------|--------|--------|']
        for r3, r5 in regressions[:30]:
            lines.append(f'| {r3["id"][:12]} | {r5["target_op"]} | {r3["ops"]:,} | {r5["ops"]:,} |')
        if len(regressions) > 30:
            lines.append(f'| ... | | | |')

    TIMEOUT_THRESHOLD = 1.9
    v5_recs_by_id = {r['id']: r for r in all_records['v5']}

    lines += ['', '## v5 Failure Classification', '',
    ...
    for op_name in SUPPORTED_OPS:
        ...
        if not op_probs:
            continue
        correct = sum(1 for p in op_probs if v5_recs_by_id[p.id]['correct'])
        timeout = sum(1 for p in op_probs
                      if not v5_recs_by_id[p.id]['correct']
                      and v5_recs_by_id[p.id]['answer'] is None
                      and v5_recs_by_id[p.id]['elapsed'] >= TIMEOUT_THRESHOLD)
        ...
        wrong_s = sum(1 for p in op_probs
                      if not v5_recs_by_id[p.id]['correct']
                      and v5_recs_by_id[p.id]['answer'] is not None)
