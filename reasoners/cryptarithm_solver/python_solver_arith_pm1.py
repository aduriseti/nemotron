"""
python_solver_arith_pm1.py — v5: branched from v3 (python_solver_arith.py).

Adds support for ±1 ops on top of v3's {add, sub, mul, cat}:
  - add1   (L + R + 1)
  - addm1  (L + R - 1)
  - mul1   (L * R + 1)
  - mulm1  (L * R - 1)

Each new op shares the same column-derivation skeleton as its parent (add or
mul), with a constant +1/-1 shift in the units column. No extra branching.

Edge cases (L + R == 0 → -1 for addm1; L * R == 0 → -1 for mulm1) are handled
only via the algebraic-inversion path; column derivation skips them.
"""
...
SUPPORTED_OPS = frozenset({'add', 'sub', 'mul', 'add1', 'addm1', 'mul1', 'mulm1'})
# Empirical frequency order (from empirical_ordering_report.md): add, sub, mul,
# addm1, mul1, add1, mulm1
OP_ORDER = ['add', 'sub', 'mul', 'addm1', 'mul1', 'add1', 'mulm1']
