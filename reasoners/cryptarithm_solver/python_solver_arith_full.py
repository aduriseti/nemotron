"""
python_solver_arith_full.py — v8: branched from v7.

Adds support for max_mod_min on top of v7's
{add, sub, mul, cat, add1, addm1, mul1, mulm1, sub_abs, sub_neg_abs}.

max_mod_min has no per-column structure (it's a value-level mod), so b0/b1
are enumerated and verified via _derive_output. The algebraic-inversion
path narrows R candidates to those satisfying max(L,R) % min(L,R) == result.

This brings v8 to feature parity with the full MATH_OPS table.
"""
...
SUPPORTED_OPS = frozenset({
    ...
    'sub_abs', 'sub_neg_abs', 'max_mod_min',
})
# Empirical frequency order (from empirical_ordering_report.md):
# add, sub, mul, sub_abs, addm1, cat, mul1, max_mod_min, sub_neg_abs, add1, mulm1
OP_ORDER = ['add', 'sub', 'mul', 'sub_abs', 'addm1', 'cat',
            'mul1', 'max_mod_min', 'sub_neg_abs', 'add1', 'mulm1']
...
def _step_cat(op_name, L, a0, a1, B0s, B1s, out_syms, op_sym, f_type,
              dm, used, op_assign, examples, ex_idx, plausible,
              solutions, deadline, max_solutions):
    """cat(L, R) = int(str(L) + str(R)). 4-digit result; output digits are
    exactly the 4 operand digits in formatter-dependent order. Algebraic
    inversion when output is fully decoded; otherwise enumerate b1 then b0
    and verify via _derive_output."""
    if len(out_syms) != 4:
        return

    # 1. Algebraic inversion: output fully decoded
    result = _decode_result_numeric(out_syms, op_sym, f_type, dm)
    if result is not None:
        if not (1000 <= result <= 9999):
            # cat output is always 4 digits with non-zero leading digit
            return
        L_top = result // 100
        R_bot = result % 100
        if L_top != L:
            return
        dm2, used2, ok = _try_assign(B0s, R_bot // 10, dm, used)
        if not ok:
            return
        dm3, used3, ok = _try_assign(B1s, R_bot % 10, dm2, used2)
        if not ok:
            return
        _search_constrained(examples, ex_idx + 1, dm3, used3,
                            {**op_assign, op_sym: op_name},
    ...
    # 2. Column derivation: enumerate (b1, b0) and verify via _derive_output
    if B1s in dm:
        b1_list = [dm[B1s]]
    else:
        b1_list = [v for v in B1_ORDER if v not in used]
    ...
    for b1 in b1_list:
        ...
        for b0 in b0_list:
            ...
            R = b0 * 10 + b1
            result_val = L * 100 + R
            new_out, ok = _derive_output(result_val, out_syms, f_type,
                                         op_name, op_sym, dm_b0, used_b0)
            if not ok:
                continue
            dm_f = {**dm_b0, **new_out}
...
def _max_mod_min_eval(L, R):
    if min(L, R) == 0:
        return max(L, R)
    return max(L, R) % min(L, R)


def _step_max_mod_min(op_name, L, a0, a1, B0s, B1s, out_syms, op_sym, f_type,
                      dm, used, op_assign, examples, ex_idx, plausible,
                      solutions, deadline, max_solutions):
    """No column structure for mod — enumerate (b0, b1) and verify result.
    When output is fully decoded, narrow R candidates using the result value."""
    # 1. Algebraic inversion: output decoded, narrow R candidates
    result = _decode_result_numeric(out_syms, op_sym, f_type, dm)
    if result is not None:
        if result < 0:
            return
        R_candidates = [R for R in range(100) if _max_mod_min_eval(L, R) == result]
    ...
    # 2. Enumerate (b1, b0) and verify via _derive_output
    if B1s in dm:
        b1_list = [dm[B1s]]
    else:
        b1_list = [v for v in B1_ORDER if v not in used]
    ...
    for b1 in b1_list:
        ...
        for b0 in b0_list:
            ...
            R = b0 * 10 + b1
            result_val = _max_mod_min_eval(L, R)
            new_out, ok = _derive_output(result_val, out_syms, f_type,
                                         op_name, op_sym, dm_b0, used_b0)
            if not ok:
                continue
...
def _step_op(op_name, L, a0, a1, B0s, B1s, out_syms, op_sym, f_type,
    ...
    if op_name == 'cat':
        _step_cat(op_name, L, a0, a1, B0s, B1s, out_syms, op_sym, f_type,
                  dm, used, op_assign, examples, ex_idx, plausible,
                  solutions, deadline, max_solutions)
        return

    if op_name == 'max_mod_min':
        _step_max_mod_min(op_name, L, a0, a1, B0s, B1s, out_syms, op_sym, f_type,
                          dm, used, op_assign, examples, ex_idx, plausible,
                          solutions, deadline, max_solutions)
        return
