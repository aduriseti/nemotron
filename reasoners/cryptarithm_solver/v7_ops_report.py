"""
Generate ops-per-puzzle distribution report for v3 / v6 / v7.
Writes markdown to v7_ops_distribution.md.

v7 promotes cat to a first-class example operator on top of v6.
"""
...
OUT_PATH = '/workspaces/nemotron/reasoners/cryptarithm_solver/v7_ops_distribution.md'
...
def main():
    ...
    lines = [
        ...
        f'Problems: {n} '
        '(add/sub/mul/cat/add1/addm1/mul1/mulm1/sub_abs/sub_neg_abs targets)', '',
    ...
    lines += ['## Per-Puzzle Speedup (v7 vs v6)', '',
              '| Speedup bucket | Count | % |',
              '|----------------|-------|---|']
    speedups = []
    for r6, r7 in zip(all_records['v6'], all_records['v7']):
        if r6['ops'] > 0:
            speedups.append(r6['ops'] / max(r7['ops'], 1))
    ...
    regressions_v6_v7 = [(r6, r7) for r6, r7 in zip(all_records['v6'], all_records['v7'])
                         if r6['correct'] and not r7['correct']]
    v6_correct = sum(1 for r in all_records['v6'] if r['correct'])
    lines += [f'## Regressions (v6 correct, v7 wrong)', '',
              f'Total: {len(regressions_v6_v7)} of {v6_correct} v6-correct problems', '']
    if regressions_v6_v7:
        lines += ['| Problem ID | Target op | v6 ops | v7 ops |',
                  '|------------|-----------|--------|--------|']
        for r6, r7 in regressions_v6_v7[:30]:
            lines.append(f'| {r6["id"][:12]} | {r7["target_op"]} | {r6["ops"]:,} | {r7["ops"]:,} |')
        if len(regressions_v6_v7) > 30:
            lines.append(f'| ... | | | |')

    TIMEOUT_THRESHOLD = 1.9
    v7_recs_by_id = {r['id']: r for r in all_records['v7']}

    lines += ['', '## v7 Failure Classification', '',
    ...
    for op_name in SUPPORTED_OPS:
        ...
        if not op_probs:
            continue
        correct = sum(1 for p in op_probs if v7_recs_by_id[p.id]['correct'])
        timeout = sum(1 for p in op_probs
                      if not v7_recs_by_id[p.id]['correct']
                      and v7_recs_by_id[p.id]['answer'] is None
                      and v7_recs_by_id[p.id]['elapsed'] >= TIMEOUT_THRESHOLD)
        ...
        wrong_s = sum(1 for p in op_probs
                      if not v7_recs_by_id[p.id]['correct']
                      and v7_recs_by_id[p.id]['answer'] is not None)
