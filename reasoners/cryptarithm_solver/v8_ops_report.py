"""
Generate ops-per-puzzle distribution report for v3 / v7 / v8.
Writes markdown to v8_ops_distribution.md.

v8 adds max_mod_min support on top of v7. v8 is feature-complete: it covers
every op in MATH_OPS.
"""
...
import reasoners.cryptarithm_solver.python_solver_arith_pm1_abs_cat as _ps7
import reasoners.cryptarithm_solver.python_solver_arith_full as _ps8
from reasoners.cryptarithm_solver.python_solver_arith import solve_v3
from reasoners.cryptarithm_solver.python_solver_arith_pm1_abs_cat import solve_v7
...
OUT_PATH = '/workspaces/nemotron/reasoners/cryptarithm_solver/v8_ops_distribution.md'
...
def run_variant(problems, label, solve_fn):
    ...
    for p in tqdm(problems, desc=label, unit='prob'):
        ...
        _ps7._derive_output = _counted
        _ps8._derive_output = _counted
        try:
            t0 = time.time()
            ...
            _ps7._derive_output = orig
            _ps8._derive_output = orig
...
def main():
    ...
    lines = [
        ...
        f'Problems: {n} (all 11 supported target ops)', '',
    ...
    lines += ['## Per-Puzzle Speedup (v8 vs v7)', '',
              '| Speedup bucket | Count | % |',
              '|----------------|-------|---|']
    speedups = []
    for r7, r8 in zip(all_records['v7'], all_records['v8']):
        if r7['ops'] > 0:
            speedups.append(r7['ops'] / max(r8['ops'], 1))
    ...
    regressions_v7_v8 = [(r7, r8) for r7, r8 in zip(all_records['v7'], all_records['v8'])
                         if r7['correct'] and not r8['correct']]
    v7_correct = sum(1 for r in all_records['v7'] if r['correct'])
    lines += [f'## Regressions (v7 correct, v8 wrong)', '',
              f'Total: {len(regressions_v7_v8)} of {v7_correct} v7-correct problems', '']
    if regressions_v7_v8:
        lines += ['| Problem ID | Target op | v7 ops | v8 ops |',
                  '|------------|-----------|--------|--------|']
        for r7, r8 in regressions_v7_v8[:30]:
            lines.append(f'| {r7["id"][:12]} | {r8["target_op"]} | {r7["ops"]:,} | {r8["ops"]:,} |')
        if len(regressions_v7_v8) > 30:
            lines.append(f'| ... | | | |')

    TIMEOUT_THRESHOLD = 1.9
    v8_recs_by_id = {r['id']: r for r in all_records['v8']}

    lines += ['', '## v8 Failure Classification', '',
    ...
    for op_name in SUPPORTED_OPS:
        ...
        if not op_probs:
            continue
        correct = sum(1 for p in op_probs if v8_recs_by_id[p.id]['correct'])
        timeout = sum(1 for p in op_probs
                      if not v8_recs_by_id[p.id]['correct']
                      and v8_recs_by_id[p.id]['answer'] is None
                      and v8_recs_by_id[p.id]['elapsed'] >= TIMEOUT_THRESHOLD)
        ...
        wrong_s = sum(1 for p in op_probs
                      if not v8_recs_by_id[p.id]['correct']
                      and v8_recs_by_id[p.id]['answer'] is not None)
